...
import type { OutcomeRecord, RawConfidence, RawConfidenceFactors } from './types'
...
export class RawConfidenceCalculator {
  ...
  private calculatePredictionStability(components: RelevanceScore['components']): number {
    ...
    const variance = perturbedScores.reduce((sum, v) => sum + (v - mean) ** 2, 0) / perturbedScores.length
    const stdDev = Math.sqrt(variance)
  ...
  private calculateHistoricalAccuracy(
    ...
    const relevant = this.outcomeHistory.filter((record) => {
      ...
      if (record.possibility.type !== possibilityType)
        return false

      // Similar environment
      if (record.contextFeatures['environment.type'] !== context.environment.type)
        return false
