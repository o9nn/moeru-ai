...
import type { CognitiveContext, Possibility, RegulationContext } from '@proj-airi/cognitive-core'
...
export class AionCharacter {
  ...
  constructor(config: AionConfigInput = {}) {
    this.config = cloneAionConfig({
      ...defaultAionConfig,
      ...config,
      traits: {
        ...defaultAionConfig.traits,
        ...config.traits,
      },
    })
    this.state = cloneAionState(initialQuantumCognitiveState)
    this.state.activeDimensions = this.config.dimensionality
  ...
  getPersonality(): {
    ...
    return {
      ...
      config: cloneAionConfig(this.config),
    }
  }
  ...
  async processInput(input: string): Promise<{
    ...
    return {
      ...
      probabilityBranches: structuredClone(branches),
    }
  }
  ...
  async makeQuantumDecision(
    ...
    if (scored.length === 0) {
      throw new Error('A quantum decision requires at least one possibility')
    }
...
Please provide a quantum reflection following the template above.`
  ...
  getState(): Readonly<QuantumCognitiveState> {
    return cloneAionState(this.state)
  }
  ...
  getConfig(): Readonly<AionConfig> {
    return cloneAionConfig(this.config)
  }

  /**
   * Export a detached, versioned snapshot suitable for JSON persistence.
   * Wisdom metrics are intentionally session-scoped and are not serialized.
   */
  exportSnapshot(): AionSnapshotV1 {
    return createAionSnapshot(this.config, this.state)
  }

  /**
   * Export a snapshot as JSON.
   */
  exportSnapshotJson(indentation = 0): string {
    return serializeAionSnapshot(this.exportSnapshot(), indentation)
  }

  /**
   * Restore state from a validated snapshot object or JSON string.
   */
  restoreSnapshot(input: unknown): void {
    const snapshot = parseAionSnapshot(input)
    this.config = cloneAionConfig(snapshot.config)
    this.state = cloneAionState(snapshot.state)
    this.wisdom = new WisdomTracker(this.config.name)
  }

  /**
   * Construct an Aion character from a persisted snapshot.
   */
  static fromSnapshot(input: unknown): AionCharacter {
    const snapshot = parseAionSnapshot(input)
    const character = new AionCharacter(snapshot.config)
    character.restoreSnapshot(snapshot)
    return character
  }
