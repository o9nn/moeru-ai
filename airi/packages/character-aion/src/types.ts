...
/** Constructor overrides with independently optional personality traits. */
export type AionConfigInput = Omit<Partial<AionConfig>, 'traits'> & {
  traits?: Partial<AionConfig['traits']>
}
...
/**
 * Stable, JSON-safe persistence envelope for Aion's identity and cognitive state.
 * Increment the version before making an incompatible schema change.
 */
export interface AionSnapshotV1 {
  version: 1
  character: 'aion'
  createdAt: number
  config: AionConfig
...
export type AionSnapshot = AionSnapshotV1
